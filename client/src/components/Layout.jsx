...
import { Trophy, BookOpen, User, Home, ShieldAlert, LogOut, GraduationCap, Music2, Volume2, VolumeX, BookCheck, User2, Gamepad2, HelpCircle, Sun, Moon, PencilRuler } from 'lucide-react'
            ...
            <aside className="w-64 bg-secondary border-r border-card-border flex flex-col flex-shrink-0 z-40 transition-colors duration-300">
                ...
                <div className="p-6 border-b border-card-border flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-bold text-sm text-main leading-tight">AAIJ</p>
                                ...
                                <div className="ml-3 pl-3 border-l border-white/10 space-y-1 mt-1">
                                    {adminItems.slice(1).map(item => (
                                        <button
                                            key={item.path}
                                            onClick={() => handleNavigate(item.path)}
                                            className={`w-full text-left ${isActive(item.path) ? 'sub-nav-item-active' : 'sub-nav-item'}`}
                                        >
