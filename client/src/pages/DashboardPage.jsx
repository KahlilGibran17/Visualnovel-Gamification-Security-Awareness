...
import { Trophy, Star, BookOpen, Award, Flame, Lock, CheckCircle, PlayCircle, ChevronRight, TrendingUp, Circle, MailWarning, ShieldCheck } from 'lucide-react'
...
const renderIcon = (iconName) => {
    if (iconName === 'BookOpen') return <BookOpen className="w-6 h-6 text-main" />
    if (iconName === 'MailWarning') return <MailWarning className="w-6 h-6 text-main" />
    if (iconName === 'ShieldCheck') return <ShieldCheck className="w-6 h-6 text-main" />
    if (iconName === 'Circle') return <Circle className="w-6 h-6 text-main" />
    // Fallback for emojis
    return <span className="text-xl drop-shadow-md">{iconName || '🔘'}</span>
}
...
function ChapterMapNode({ chapter, progress, locked, onPlay, index, isNoXpRole }) {
    ...
    return (
        ...
        >
            {/* Step Badge */}
            <div className={`text-[9px] font-black tracking-widest uppercase mb-2 px-2 py-0.5 rounded-md border select-none ${
                isComplete ? 'text-accent border-accent/20 bg-accent/5 shadow-[0_0_10px_rgba(255,214,10,0.15)]' :
                isUnlocked ? 'text-primary border-primary/20 bg-primary/5 shadow-[0_0_10px_rgba(230,57,70,0.15)]' : 'text-dim border-white/5 bg-transparent'
            }`}>
                STEP {index + 1}
            </div>

            <motion.button
                id={`chapter-${chapter.chapter_id || chapter.id}-btn`}
                onClick={() => isUnlocked && onPlay(chapter)}
                disabled={!isUnlocked}
            ...
            <div className="text-center max-w-[130px] flex flex-col items-center">
                <p className="text-xs font-semibold text-main leading-tight mt-2">{chapter.title}</p>
                <p className="text-[10px] text-dim leading-normal mt-0.5">
                    {chapter.subtitle || (chapter.isElearning ? 'Pembelajaran Interaktif' : chapter.location || 'Misi Utama')}
                </p>

                {isComplete && !isNoXpRole && (
                    <p className="text-xs text-accent font-bold mt-1.5">+{progress.xpEarned || 0} XP</p>
                )}
            </div>
        </motion.div>
    )
...
export default function DashboardPage() {
    ...
    const { chapterProgress, getLevelFromXP, getNextLevel, CHAPTERS, ROADMAP_NODES, BADGES, LEVELS, loading, error, leaderboard, getUserRank, getNextRankGap, elearningCompleted } = useGame()
    ...
    const rawNodes = (ROADMAP_NODES || []).map(node => {
        ...
        const isCompleted = node.node_type === 'E-Learning'
    ...
    const dashboardNodes = rawNodes.map((n, idx) => {
        const isLocked = idx === 0 ? false : !rawNodes[idx - 1].isCompleted
        return {
            ...n,
            isLocked
        }
    ...
    const gameChapters = CHAPTERS.filter(c => c.type !== 'E-Learning')
    const totalChapters = gameChapters.length || 6
    const completedChapters = gameChapters.filter(c => chapterProgress[c.id]?.completed).length
    ...
    return (
        <Layout>
            ...
            <div className="p-6 space-y-6 max-w-7xl mx-auto">
                ...
                {/* Stat Widgets */}
                <div className={`grid ${isNoXpRole ? 'grid-cols-1' : 'grid-cols-2 lg:grid-cols-4'} gap-4`}>
                    ...
                    <StatWidget icon={BookOpen} label="Modul Selesai" value={`${completedChapters}/${totalChapters}`} color="#60a5fa" delay={0.2} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    >
                        ...
                        <div className="flex flex-col md:flex-row items-center md:items-start justify-center gap-4 py-4 px-2 relative z-10">
                            {dashboardNodes.map((node, idx) => {
                                const locked = node.isLocked
                                const lockedNext = idx === dashboardNodes.length - 1 ? false : dashboardNodes[idx + 1].isLocked
                                
                                return (
                                    <Fragment key={node.id}>
                                        <ChapterMapNode
                                            chapter={node}
                                            progress={{ completed: node.isCompleted, xpEarned: node.xpEarned }}
                                            locked={locked}
                                            onPlay={handlePlayNode}
                                            index={idx}
                                            isNoXpRole={isNoXpRole}
                                        />
                                        {idx < dashboardNodes.length - 1 && (
                                            <Fragment>
                                                {/* Desktop Horizontal Connection Line */}
                                                <div className="hidden md:flex flex-1 items-center justify-center min-w-[30px] lg:min-w-[60px] mt-[54px] z-0">
                                                    <div className={`h-1.5 w-full rounded-full transition-all duration-500 ${
                                                        lockedNext 
                                                            ? 'bg-white/[0.06] border-t border-dashed border-white/10' 
                                                            : 'bg-gradient-to-r from-accent to-primary shadow-[0_0_15px_rgba(255,214,10,0.5)]'
                                                    }`} />
                                                </div>
                                                {/* Mobile Vertical Connection Line */}
                                                <div className="flex md:hidden flex-col items-center py-1 z-0">
                                                    <div className={`w-1.5 h-8 rounded-full transition-all duration-500 ${
                                                        lockedNext 
                                                            ? 'bg-white/[0.06] border-l border-dashed border-white/10' 
                                                            : 'bg-gradient-to-b from-accent to-primary shadow-[0_0_15px_rgba(255,214,10,0.5)]'
                                                    }`} />
                                                </div>
                                            </Fragment>
                                        )}
