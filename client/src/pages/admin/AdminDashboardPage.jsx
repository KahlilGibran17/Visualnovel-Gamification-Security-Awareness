...
import { 
    Users, BookOpen, BarChart2, Bell, TrendingUp, 
    AlertCircle, CheckCircle, Clock, GraduationCap, PlusCircle 
} from 'lucide-react'
...
export default function AdminDashboardPage() {
    ...
    const loadRecentActivity = useCallback(async (showLoading = false) => {
        ...
        try {
            const res = await axios.get('/api/admin/recent-activity', { params: { limit: 8 } })
        ...
        const loadAdminData = async () => {
            ...
            try {
                const [leaderboardRes, deptRes, chaptersRes] = await Promise.all([
                    ...
                    axios.get('/api/progress/chapters/total'), // Use reliable chapter count
                ])
    ...
    const departmentProgress = useMemo(() => {
        ...
        const grouped = leaderboardRows.reduce((acc, user) => {
            ...
            if (isAllChaptersDone(user.chaptersCompleted)) acc[dept].completed += 1
            return acc
        }, {})
            ...
            <div className="p-6 max-w-6xl mx-auto space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        ...
                        { icon: GraduationCap, label: 'E-Learning', desc: 'Manage module lessons and learning materials', path: '/admin/elearning', color: '#ec4899' },
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <motion.div className="glass-card p-5"
                        ...
                        <div className="space-y-3">
                            {loading ? (
                                <p className="text-muted text-sm italic">Loading department stats...</p>
                            ) : departmentProgress.length === 0 ? (
                                <p className="text-muted text-sm italic">No department data available.</p>
                            ) : departmentProgress.map(d => (
                                <div key={d.dept} className="flex items-center gap-3">
                                    <span className="w-20 text-muted text-sm truncate">{d.dept}</span>
                    ...
                    <motion.div className="glass-card p-5"
                        ...
                        <div className="space-y-3">
                            {loadingActivity ? (
                                <p className="text-muted text-sm italic">Loading recent activity...</p>
                                ...
                                <div key={`${a.id}-${i}`} className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-main text-sm font-medium truncate">{a.userName}</p>
                                        <p className="text-muted text-xs truncate">{a.action}</p>
                                    </div>
