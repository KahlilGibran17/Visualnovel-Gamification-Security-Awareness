...
router.get('/admin/getEmployees', requireAuth, requireRole('super-admin'), async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT
                u.id,
        ...
        console.error('GET /api/users/admin/getEmployees error:', err)
        res.status(500).json({ message: 'Server error' })
    }
})

router.get('/admin/getAdmins', requireAuth, requireRole('super-admin'), async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT
                u.id,
             ...
             JOIN roles r ON r.id = u.role_id
             WHERE r.name = 'admin'
             ORDER BY u.created_at DESC, COALESCE(u.display_name, u.name) ASC`
        )

        res.json(result.rows)
    } catch (err) {
        console.error('GET /api/users/admin/getAdmins error:', err)
        res.status(500).json({ message: 'Server error' })
    }
})

router.patch('/admin/updateRole/:id', requireAuth, requireRole('super-admin'), async (req, res) => {
    const parsedUserId = Number(req.params.id)
    const requestedRoleName = String(req.body?.role_name || '').trim()
    const requestedRoleId = Number(req.body?.role_id)
    ...
    try {
        ...
        if (Number.isInteger(requestedRoleId) && requestedRoleId > 0) {
            ...
            const normalizedRoleName = requestedRoleName || 'admin'
            ...
            targetRoleId = Number(roleResult.rows[0].id)
        }
        ...
        const updateResult = await pool.query(
            `UPDATE users
             SET role_id = $1,
                 updated_at = NOW()
             WHERE id = $2
             RETURNING id, nik, COALESCE(display_name, name) AS name, email, department, position, role_id`,
            [targetRoleId, parsedUserId]
        )
        ...
        console.error('PATCH /api/users/admin/updateRole/:id error:', err)
        res.status(500).json({ message: 'Server error' })
    }
})
